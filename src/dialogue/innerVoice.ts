...
const QUOTES = [
  '작은 별 하나를 돌보는 마음이, 너를 오늘도 환하게 해.',
  '천천히 다가가면, 어느새 우리는 친구가 돼.',
  '마른 자리에도 어딘가 시원한 우물이 숨어 있어.',
  '오늘 네가 쏟은 다정한 시간이, 이 별을 특별하게 만들어.',
  '서두르지 않아도 괜찮아. 별은 네 속도로 자라니까.',
  '눈을 감고 가만히 들어봐. 마음이 먼저 알아챌 거야.',
]
...
const QUESTIONS: Q[] = [
  ...
  { text: '지금 가장 가까워지고 싶은 마음은 어떤 거야?', pick: (p) => 0.5 + p.sn * 0.7 },
...
const EMPATHY: Record<Mood, { f: string; t: string }> = {
  ...
  lonely:  { f: '외로웠구나. 그래도 봐, 우리가 이렇게 마주 앉아 있잖아. 너는 혼자가 아니야.', t: '관계는 천천히 다가가는 데서 시작돼. 한 사람에게 한 걸음 가까이 가보는 건 어때?' },
  ...
  love:    { f: '누군가를 소중히 여기는 그 마음이 바로 너의 정원이야. 그 시간이 너를 특별하게 만들어.', t: '소중한 걸 소중하다고 말할 수 있는 건 용기야.' },
...
export function greeting(p: Personality, name: string): string {
  ...
  return `안녕, 나는 네 안의 작은 별이야. ${who}에 온 걸 환영해. (${code})\n편하게 앉아서, 오늘의 너를 들려줄래?`
}
